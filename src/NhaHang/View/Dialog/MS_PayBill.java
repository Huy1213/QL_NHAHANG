...
public class MS_PayBill extends javax.swing.JDialog {
    ...
    private int idKH;
    private ArrayList<Integer> listIDMonAn = new ArrayList<>();
    ...
    public MS_PayBill(java.awt.Frame parent, boolean modal) {
        ...
        TableActionEvent event = new TableActionEvent() {
            ...
            public void onDelete(int row) {
                if (row >= 0 && row < cthd.getRowCount()) {
                    int idMonAn = listIDMonAn.get(row); // Lấy ID_MonAn từ danh sách

                    try {
                        // Xóa khỏi cơ sở dữ liệu
                        service.deleteCTHD(idHoaDon, idMonAn);

                        // Xóa hàng khỏi bảng và danh sách ID
                        ((DefaultTableModel) cthd.getModel()).removeRow(row);
                        listIDMonAn.remove(row);

                        // Cập nhật lại bảng sau khi xóa thành công
                        customer = new ModelKhachHang();
                        customer.setID_KH(idKH);
                        HoaDon = service.FindHoaDon(customer);
                        showBill(HoaDon);

                        System.out.println("Đã xóa dòng với ID_MonAn: " + idMonAn);
                    ...
                    System.out.println("Hàng không hợp lệ: " + row);
                }
            }
        };

        cthd.getColumnModel().getColumn(3).setCellRenderer(new TableActionCellRenderDelete());
        cthd.getColumnModel().getColumn(3).setCellEditor(new TableActionCellEditorDelete(event));
    ...
    public void showBill(ModelHoaDon hoadon) {
        try {
            ...
            DefaultTableModel model = (DefaultTableModel) cthd.getModel();
            ...
            for (ModelCTHD data : listCTHD) {
                ...
                model.addRow(new Object[]{data.getTenMonAn(), data.getSoluong(), df.format(data.getThanhTien()) + "đ"});

                // Thêm ID_MonAn vào danh sách
                listIDMonAn.add(data.getID_MonAn());
            }
    ...
    private void initComponents() {
        ...
        cthd.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Món ăn", "Số lượng", "Thành tiền", "Xóa"
            }
        ) {
