...
import NhaHang.View.Dialog.MS_Confirm_DatBan;
...
public class CardBan extends javax.swing.JPanel {
    ...
    private MS_Confirm_DatBan obj1;
            ...
            switch (data.getStatus()) {
                ...
                cmdBook.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        obj1 = new MS_Confirm_DatBan(Main_Customer_Frame.getFrames()[0], true);                     
                        boolean isConfirmed = obj1.Cus_ConfirmBook(data, customer); // Lấy kết quả xác nhận từ hộp thoại

                        if (isConfirmed) {
                            try {
                                // Cập nhật trạng thái bàn khi người dùng đã xác nhận đặt bàn
                                serviceS.updatetrangthaidangdung(data.getID());
                                loadUpdatedTableStatus();
                            } catch (java.sql.SQLException ex) {
                                Logger.getLogger(CardBan.class.getName()).log(Level.SEVERE, null, ex);
                            }
                        } else {
                            System.out.println("Người dùng đã hủy đặt bàn.");
                        }
                    }
   ...
   private boolean checkIfCustomerHasReservedTable(ModelKhachHang customer) {
    try {
        // Giả sử bạn đã có ID_Ban từ data
        return serviceS.checkCustomerReservation(data.getID(), customer.getID_KH()); // Thay đổi ở đây để truyền MaKH
        } catch (java.sql.SQLException ex) {
            Logger.getLogger(CardBan.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }
    private void loadUpdatedTableStatus() {
        // Cập nhật trạng thái mới của bàn (sau khi đã đặt)
        lbValue.setText("Dang dung bua");
        cmdBook.setText("KHÔNG THỂ ĐẶT");
        cmdBook.setBackground(Color.decode("#232526"));
        cmdBook.setEnabled(false);

        // Revalidate và repaint lại giao diện để hiển thị cập nhật
        this.revalidate();
        this.repaint();
    }
