...
public class ServiceStaff {
    ...
    public void InsertNL(ModelNguyenLieu data) throws SQLException {
        ...
        String sqlKho = "INSERT INTO Kho(ID_NL, SLTon) VALUES(?, ?)";
        PreparedStatement pKho = con.prepareStatement(sqlKho);
        pKho.setInt(1, data.getId()); // ID_NL
        pKho.setInt(2, 0); // SLTon (bạn có thể điều chỉnh giá trị này)
        pKho.execute();
    ...
    public void InsertPNK_CTNK(ModelPNK pnk, ArrayList<ModelKho> list) throws SQLException {
        // Thêm phiếu nhập kho với giá trị Tổng tiền ban đầu là 0
        String sql = "INSERT INTO PhieuNK(ID_NK,ID_NV,NgayNK,Tongtien) VALUES (?,?,CONVERT(DATE, ?, 105), 0)";
        PreparedStatement p = con.prepareStatement(sql);
        ...
        // Khai báo biến để tính tổng tiền cho phiếu nhập kho
        int tongTien = 0;
        ...
        for (ModelKho data : list) {
            if (data.getSlTon() > 0) {
                String sql_getPrice = "SELECT Dongia FROM NguyenLieu WHERE ID_NL = ?";
                PreparedStatement p_getPrice = con.prepareStatement(sql_getPrice);
                p_getPrice.setInt(1, data.getIdNL());
                ResultSet rs = p_getPrice.executeQuery();

                if (rs.next()) {
                    int gia = rs.getInt("Dongia");
                    int thanhTien = data.getSlTon() * gia;

                    // Cộng thành tiền vào tổng tiền
                    tongTien += thanhTien;

                    // Thêm chi tiết nhập kho với thành tiền
                    sql_ct = "INSERT INTO CTNK(ID_NK,ID_NL,SoLuong,ThanhTien) VALUES (?,?,?,?)";
                    ...
                    p_ct.setInt(4, thanhTien); // Tính thành tiền = Số lượng * Giá
                    p_ct.execute();
                    p_ct.close();

                    // Cập nhật số lượng tồn trong bảng Kho
                    String sql_updateKho = "UPDATE Kho SET SLTon = SLTon + ? WHERE ID_NL = ?";
                    PreparedStatement p_updateKho = con.prepareStatement(sql_updateKho);
                    p_updateKho.setInt(1, data.getSlTon()); // Thêm số lượng mới vào SLTon hiện tại
                    p_updateKho.setInt(2, data.getIdNL()); // Cập nhật cho nguyên liệu tương ứng
                    p_updateKho.executeUpdate();
                    p_updateKho.close();
                }
                rs.close();
                p_getPrice.close();
        ...
        // Cập nhật tổng tiền cho phiếu nhập kho sau khi thêm chi tiết
        String sql_updatePNK = "UPDATE PhieuNK SET Tongtien = ? WHERE ID_NK = ?";
        PreparedStatement p_update = con.prepareStatement(sql_updatePNK);
        p_update.setInt(1, tongTien);  // Gán tổng tiền vừa tính được
        p_update.setInt(2, pnk.getIdNK());
        p_update.execute();
        p_update.close();
    ...
    public void InsertPXK_CTXK(ModelPXK pxk, ArrayList<ModelKho> list) throws SQLException {
        ...
        for (ModelKho data : list) {
            if (data.getSlTon() > 0) {
                ...
                String sql_updateKho = "UPDATE Kho SET SLTon = SLTon - ? WHERE ID_NL = ?";
                PreparedStatement p_updateKho = con.prepareStatement(sql_updateKho);
                p_updateKho.setInt(1, data.getSlTon()); // Giảm số lượng hiện tại
                p_updateKho.setInt(2, data.getIdNL()); // Cập nhật cho nguyên liệu tương ứng
                p_updateKho.executeUpdate();
                p_updateKho.close();
            }
    ...
    public ArrayList<ModelKhachHang> MenuKH() throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, Ngaythamgia AS Ngay, Doanhso, Diemtichluy,SDT FROM KhachHang";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            ...
            ModelKhachHang data = new ModelKhachHang(ID_KH, name, dateJoin, sales, points, sdt);
            list.add(data);
        }
        r.close();
        p.close();
    ...
    public void insertDiemTichLuy(int maHD, int maKH, int soDiemTichLuy) throws SQLException {
        String sql = "INSERT INTO DiemTichLuy (MaHD, MaKH, SoDiemTichLuy, NgayTichLuy) VALUES (?, ?, ?, GETDATE())";
        try (PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setInt(1, maHD);           // Mã hóa đơn
            stmt.setInt(2, maKH);           // Mã khách hàng
            stmt.setInt(3, soDiemTichLuy);  // Số điểm tích lũy

            stmt.executeUpdate(); // Thực thi lệnh SQL để thêm dữ liệu vào bảng DiemTichLuy
    ...
    public boolean checkCustomerReservation(int tableId, int customerId) throws SQLException {
        String query = "SELECT * FROM HoaDon WHERE ID_Ban = ? AND ID_KH = ? AND Trangthai = 'Chua thanh toan'"; // Cập nhật tên cột cho chính xác
        try (PreparedStatement stmt = con.prepareStatement(query)) {
            stmt.setInt(1, tableId); // ID_Ban
            stmt.setInt(2, customerId); // MaKH
            ResultSet rs = stmt.executeQuery();
            return rs.next(); // Trả về true nếu có hàng
        }
    }

    public void updateTongTien(int idHoaDon, int newTongTien) throws SQLException {
        String sql = "UPDATE HoaDon SET Tongtien = ? WHERE ID_HoaDon = ?";
        try (PreparedStatement p = con.prepareStatement(sql)) {
            p.setInt(1, newTongTien);
            p.setInt(2, idHoaDon);
            int rowsAffected = p.executeUpdate();

            if (rowsAffected == 0) {
                System.out.println("Không tìm thấy hóa đơn với ID_HoaDon = " + idHoaDon);
            } else {
                System.out.println("Đã cập nhật tổng tiền thành công cho hóa đơn ID = " + idHoaDon);
            }
        } catch (SQLException e) {
